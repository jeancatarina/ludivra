...
O primeiro corte publica `RendererProfileRequest` no manifest v4 e resolve a matriz no `renderer-three` antes de construir o renderer. O BrowserHost expõe pedido/efetivo/método/adapter/motivo e a cadeia de pós-processamento no inspection surface. `desktop-high` carrega `WebGPURenderer` sob demanda e o inicializa antes de aceitar o perfil; erro de API ou de init só prossegue em `desktop-compatible` quando esse fallback está declarado, emitindo `RENDER_METHOD_FALLBACK`; sem fallback ou com feature obrigatória ausente, a inicialização falha com código estável. O caminho WebGPU requisita o adapter explicitamente, publica seus metadados quando a política do navegador permite e pede `timestamp-query`; os resultados formam uma janela limitada de 120 amostras GPU com mediana, p95 e budget de 16,67 ms exposta pela inspeção. Para `desktop-high`, `RenderPipeline` recebe o pass da cena e bloom TSL em meia resolução; o caminho WebGL mantém a cadeia cinematográfica compatível. O `game package --target steam-macos` executa o smoke desktop, exige `postprocess: webgpu-bloom` e grava seleção, pipeline e benchmark no provenance do pacote.
