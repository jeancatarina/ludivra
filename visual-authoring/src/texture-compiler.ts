...
export interface CompiledTexture {
  ...
  height: number;
  edgeDelta: number;
  issues: Array<{ code: "VISUAL_TEXTURE_NOT_TILEABLE"; message: string }>;
  maps: {
...
function encoded(width: number, height: number, data: Buffer): CompiledTextureMap {
...
function luminance(data: Buffer, pixel: number): number {
  const offset = pixel * 4;
  return ((data[offset] ?? 0) * 0.2126 + (data[offset + 1] ?? 0) * 0.7152 + (data[offset + 2] ?? 0) * 0.0722) / 255;
}

function sample(data: Buffer, width: number, height: number, x: number, y: number): number {
  const wrappedX = (x + width) % width;
  const wrappedY = (y + height) % height;
  return luminance(data, wrappedY * width + wrappedX);
}

function edgeDelta(data: Buffer, width: number, height: number): number {
  let total = 0;
  let samples = 0;
...
export function compileTexture(
  input: Uint8Array,
  ...
  const issues = request.requirements?.tileable === true && seam > 0.12
    ? [{ code: "VISUAL_TEXTURE_NOT_TILEABLE" as const, message: `Opposite-edge color delta ${seam.toFixed(4)} exceeds 0.12` }]
    : [];
  return {
    width,
    height,
