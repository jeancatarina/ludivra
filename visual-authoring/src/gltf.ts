import { SURFACE, SURFACE_NAMES, type CharacterGeometry, type SurfaceClass, type Vector3 } from "./geometry.js";
...
export function buildGltf(
  ...
  const indexGroups = SURFACE_NAMES.map(() => [] as number[]);
  for (let triangle = 0; triangle < geometry.indices.length / 3; triangle += 1) {
    ...
    group.push(
      geometry.indices[triangle * 3] ?? 0,
      geometry.indices[triangle * 3 + 1] ?? 0,
      geometry.indices[triangle * 3 + 2] ?? 0
    );
  }
  const activeSurfaces = indexGroups
    .map((indices, surface) => ({ indices, surface: surface as SurfaceClass }))
