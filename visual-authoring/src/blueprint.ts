...
const profiles: Record<HumanoidGenerationProfile, HumanoidBlueprint> = {
  "hero-mascot": {
    profile: "hero-mascot",
    skeleton: {
      hipBase: 0.34,
...
export function buildHumanoidBlueprint(
  spec: CharacterSpec,
  _style?: VisualStyleBible
): HumanoidBlueprint {
  const requested = spec.features?.generationProfile;
  const inferred: HumanoidGenerationProfile = requested ??
