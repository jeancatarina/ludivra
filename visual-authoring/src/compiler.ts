...
import { generateCharacterGeometry, type CharacterGeometry } from "./geometry.js";
...
export const VISUAL_GENERATOR_VERSION = 4;
...
export function visualCacheKey(
  ...
  return createHash("sha256")
    .update(`${VISUAL_GENERATOR_VERSION}\0${spec.seed}\0${stringifyYaml(
      { spec, style, textureHashes },
      { sortMapEntries: true }
    )}`)
    .digest("hex");
}
...
export interface CompiledCharacter {
  ...
  material: {
    ...
    proceduralLayers: Array<{ type: string; strength: number }>;
    generatedLayers: Array<{ request: string; blend: "multiply" | "overlay"; strength: number }>;
  };
}

export function compileCharacter(spec: CharacterSpec, style: VisualStyleBible): CompiledCharacter {
  ...
  return {
    ...
    material: {
      ...
      generatedLayers: spec.surfaces
        ...
        .map(({ id, kind }) => ({ request: id, blend: kind === "decal" ? "overlay" as const : "multiply" as const, strength: 0.55 }))
    }
  };
}
