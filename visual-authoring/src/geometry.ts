...
export const SURFACE = {
  ...
  glossy: 4,
  hard: 5
} as const satisfies Record<string, SurfaceClass>;

export const SURFACE_NAMES = ["skin", "cloth", "leather", "hair", "glossy", "hard"] as const;
...
export function buildSkeleton(spec: CharacterSpec, style: VisualStyleBible): BoneDefinition[] {
  ...
  const leftElbow: Vector3 = [-shoulderHalf - upperArm * lateralReach, shoulderY - upperArm * armDrop, leftDrift];
  const rightElbow: Vector3 = [shoulderHalf + upperArm * lateralReach, shoulderY - upperArm * armDrop, rightDrift];
  const leftWrist: Vector3 = [leftElbow[0] - lowerArm * (presentationPose === "relaxed" ? 0.25 : 0.9), leftElbow[1] - lowerArm * forearmDrop, leftDrift];
  const rightWrist: Vector3 = [rightElbow[0] + lowerArm * (presentationPose === "relaxed" ? 0.25 : 0.9), rightElbow[1] - lowerArm * forearmDrop, rightDrift];
...
function attachmentSegments(
  ...
  const profileHeadCenter: Vector3 = [
    0,
  ...
  if (spec.archetype.head === "human") {
    push(
      "face.left-inner-ear",
      head.index,
      add(profileHeadCenter, [-profileHeadRadius * 1.03, profileHeadRadius * 0.13, profileHeadRadius * 0.02]),
      add(profileHeadCenter, [-profileHeadRadius * 1.03, -profileHeadRadius * 0.22, profileHeadRadius * 0.05]),
      profileHeadRadius * 0.105,
      profileHeadRadius * 0.08,
      "skinHighlight"
    );
    push(
      "face.right-inner-ear",
      head.index,
      add(profileHeadCenter, [profileHeadRadius * 1.03, profileHeadRadius * 0.13, profileHeadRadius * 0.02]),
      add(profileHeadCenter, [profileHeadRadius * 1.03, -profileHeadRadius * 0.22, profileHeadRadius * 0.05]),
      profileHeadRadius * 0.105,
  ...
  if (hair !== undefined && hair.style !== "none") {
    push(
      "face.left-sideburn",
      head.index,
      add(profileHeadCenter, [-profileHeadRadius * 0.88, -profileHeadRadius * 0.02, profileHeadRadius * 0.2]),
      add(profileHeadCenter, [-profileHeadRadius * 0.91, -profileHeadRadius * 0.48, profileHeadRadius * 0.2]),
      profileHeadRadius * 0.12,
      profileHeadRadius * 0.065,
      hair.colorRole
    );
    push(
      "face.right-sideburn",
  ...
  if (headwear?.badge === "star") {
    ...
    for (let point = 0; point < starPoints.length; point += 1) {
      push(
        `headwear.badge-star.${point}`,
        crown.index,
        starPoints[point]!,
        starPoints[(point + 1) % starPoints.length]!,
        profileHeadRadius * 0.018,
        profileHeadRadius * 0.018,
        "badgeMark"
      );
    }
...
export function generateCharacterGeometry(spec: CharacterSpec, style: VisualStyleBible): CharacterGeometry {
  ...
  const renderSegments = segments.slice(skeleton.length).filter(({ name }) =>
    !name.startsWith("face.left-ear") &&
  ...
  for (let segmentIndex = 0; segmentIndex < renderSegments.length; segmentIndex += 1) {
    ...
    const surface: SurfaceClass = definition.name.includes("inner-ear")
      ? SURFACE.skin
