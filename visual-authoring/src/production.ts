...
export interface RasterProductionOutput {
  ...
  source: {
    ...
    matte: { color: string; transparentThreshold: number; opaqueThreshold: number };
...
export interface CompiledRasterOutput {
  ...
  metadata: {
    ...
    pixelsPerMeter: number;
    frames: RasterFrame[];
    animations: Array<{ id: string; frames: string[] }>;
  };
...
function parseHexColor(value: string): [number, number, number] {
  ...
  if (match === null) throw new Error(`VISUAL_ALPHA_INVALID: invalid matte color ${value}`);
  return [Number.parseInt(match[1]!, 16), Number.parseInt(match[2]!, 16), Number.parseInt(match[3]!, 16)];
}

function applyMatte(image: PNG, output: RasterProductionOutput): void {
  ...
  if (opaque <= transparent) throw new Error("VISUAL_ALPHA_INVALID: opaqueThreshold must exceed transparentThreshold");
  ...
  for (let y = 1; y < image.height - 1; y += 1) {
    for (let x = 1; x < image.width - 1; x += 1) {
      ...
      if (touchesTransparent) image.data[offset + 3] = Math.round(alpha * 0.2);
    }
  }
}

function alphaBounds(image: PNG, minX = 0, maxX = image.width): Bounds | null {
  let left = maxX;
  let right = minX - 1;
...
function occupiedColumnRuns(image: PNG): Array<[number, number]> {
  const occupied = Array.from({ length: image.width }, (_, x) => {
    let count = 0;
    for (let y = 0; y < image.height; y += 1) {
      if (image.data[(y * image.width + x) * 4 + 3]! >= 8) count += 1;
    }
    return count >= 2;
  });
  ...
  for (let x = 0; x <= occupied.length; x += 1) {
    if (occupied[x] === true && start < 0) start = x;
...
function opaqueMassHeight(image: PNG, bounds: Bounds): number {
  const rows: number[] = [];
  let total = 0;
  for (let y = bounds.y; y < bounds.y + bounds.height; y += 1) {
    let count = 0;
    for (let x = bounds.x; x < bounds.x + bounds.width; x += 1) {
      if (image.data[(y * image.width + x) * 4 + 3]! >= 64) count += 1;
    }
    rows.push(count);
    total += count;
  }
  if (total === 0) return bounds.height;
...
function copyFrame(source: PNG, target: PNG, bounds: Bounds, targetX: number, targetY: number): void {
...
function extrudeTransparentRgb(image: PNG, iterations: number): void {
...
function reportStatus(checks: ProductionCheck[]): "passed" | "failed" {
  return checks.some(({ status }) => status === "failed") ? "failed" : "passed";
}

export function compileRasterProduction(
  sourceBytes: Uint8Array,
  output: RasterProductionOutput
): CompiledRasterOutput {
  ...
  const frames = bounds.map((boundsValue, index): RasterFrame => {
...
function embeddedBuffers(gltf: GltfJson, binaryChunk?: Buffer): Buffer[] {
  return (gltf.buffers ?? []).map(({ uri }, index) => {
    if (uri === undefined && index === 0 && binaryChunk !== undefined) return binaryChunk;
    if (uri === undefined || !uri.startsWith("data:")) throw new Error("VISUAL_3D_DEPENDENCY_MISSING");
    const marker = uri.indexOf(",");
    return Buffer.from(uri.slice(marker + 1), uri.slice(0, marker).includes(";base64") ? "base64" : "utf8");
  });
}

function parseModelBytes(source: Uint8Array): { gltf: GltfJson; binaryChunk?: Buffer } {
  ...
  if (bytes.length >= 12 && bytes.readUInt32LE(0) === 0x46546c67) {
    ...
    while (offset + 8 <= bytes.length) {
      ...
      if (type === 0x4e4f534a) json = chunk.toString("utf8").replaceAll(/\0+$/g, "").trim();
      else if (type === 0x004e4942) binaryChunk = chunk;
      offset += 8 + length;
    }
    if (json === null) throw new Error("VISUAL_SPEC_INVALID: GLB has no JSON chunk");
    return { gltf: JSON.parse(json) as GltfJson, ...(binaryChunk === undefined ? {} : { binaryChunk }) };
  }
  return { gltf: JSON.parse(bytes.toString("utf8")) as GltfJson };
}

const componentSizes: Record<number, number> = { 5120: 1, 5121: 1, 5122: 2, 5123: 2, 5125: 4, 5126: 4 };
const typeSizes: Record<string, number> = { SCALAR: 1, VEC2: 2, VEC3: 3, VEC4: 4, MAT4: 16 };

function accessorValues(gltf: GltfJson, buffers: Buffer[], index: number): number[][] {
  const accessor = gltf.accessors?.[index];
  if (accessor === undefined || typeof accessor.bufferView !== "number") return [];
  ...
  if (componentSize === undefined || components === undefined) return [];
  ...
  for (let item = 0; item < count; item += 1) {
    ...
    for (let component = 0; component < components; component += 1) {
      ...
      if (componentType === 5121) row.push(data.readUInt8(offset));
      else if (componentType === 5123) row.push(data.readUInt16LE(offset));
      else if (componentType === 5125) row.push(data.readUInt32LE(offset));
      else if (componentType === 5126) row.push(data.readFloatLE(offset));
      else if (componentType === 5120) row.push(data.readInt8(offset));
      else row.push(data.readInt16LE(offset));
    }
    values.push(row);
  }
  return values;
...
function imageDimensions(gltf: GltfJson, buffers: Buffer[]): Array<[number, number]> {
  ...
  for (const image of gltf.images ?? []) {
    ...
    if (image.uri?.startsWith("data:")) {
      ...
      if (view !== undefined && data !== undefined) bytes = data.subarray(view.byteOffset ?? 0, (view.byteOffset ?? 0) + view.byteLength);
    }
...
export function inspectProductionGltf(
  source: string,
  output: GltfProductionOutput
): ProductionValidationReport {
  return inspectProductionGltfBytes(Buffer.from(source, "utf8"), output);
}

export function inspectProductionGltfBytes(
  source: Uint8Array,
  output: GltfProductionOutput
): ProductionValidationReport {
  ...
  for (const primitive of primitives) {
    ...
    if (weights.length > 0 && joints.length === weights.length) skinnedPrimitives += 1;
  ...
  for (const check of checks) if (check.status === "passed") delete check.code;
  return {
    schemaVersion: 1,
    profile: output.profile,
    quality: output.quality,
...
export function productionCacheKey(
