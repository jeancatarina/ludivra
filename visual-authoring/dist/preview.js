...
function projected(point, view) {
    ...
    if (view === "three-quarter")
        return [point[0] * 0.72 + point[2] * 0.48, point[1]];
    return [point[0], point[1]];
}
function drawBone(bone, view, originX, originY, scale, color) {
...
function viewPanel(segments, view, x, color, accent, heightM) {
    const originX = x + 130;
    const originY = 360;
...
function turntable(segments, x, y, color, heightM) {
    const frames = [];
...
export function renderCharacterPreview(spec, style, geometry, report) {
    const base = style.palette[spec.skin] ?? style.palette.skin ?? "#7c9d5d";
    const accent = style.palette.accent ?? "#8df0c5";
    const panels = ["front", "back", "side", "three-quarter"]
        .map((view, index) => viewPanel(geometry.segments, view, 32 + index * 278, base, accent, spec.anatomy.heightM))
        .join("");
    const clips = spec.animations.map((animation, index) => `<g transform="translate(${36 + index * 126} 555)"><rect width="112" height="30" rx="15" fill="#193044"/><circle cx="17" cy="15" r="4" fill="${accent}"/><text x="30" y="20" fill="#d9e8ef" font-size="13" font-family="system-ui">${escape(animation)}</text></g>`).join("");
    return [
