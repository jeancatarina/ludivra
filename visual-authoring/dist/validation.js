...
function triangleAreaSquared(geometry, offset) {
...
function pointSegmentDistance(point, start, end) {
    ...
    const t = Math.min(1, Math.max(0, (offset[0] * direction[0] + offset[1] * direction[1] + offset[2] * direction[2]) / lengthSquared));
...
function approximateIntersectionRatio(geometry) {
    let candidates = 0;
    let intersections = 0;
    for (let leftIndex = 0; leftIndex < geometry.segments.length; leftIndex += 1) {
        ...
        for (let rightIndex = leftIndex + 1; rightIndex < geometry.segments.length; rightIndex += 1) {
            ...
            if (right === undefined || shareEndpoint(left, right))
                continue;
...
function eyeNoseClearance(spec, geometry, blueprint) {
    ...
    if (centerDistance <= 1e-9)
        return Number.NEGATIVE_INFINITY;
    const direction = delta.map((component) => component / centerDistance);
    const noseAxes = [
    ...
    const noseSupport = Math.hypot(noseAxes[0] * direction[0], noseAxes[1] * direction[1], noseAxes[2] * direction[2]);
    return centerDistance - noseSupport - height * blueprint.face.eyeRadius;
}
export function validateCharacter(spec, style, geometry) {
    const blueprint = buildHumanoidBlueprint(spec, style);
    const triangles = geometry.indices.length / 3;
    let degenerateTriangles = 0;
    for (let offset = 0; offset < geometry.indices.length; offset += 3) {
        if (triangleAreaSquared(geometry, offset) < 1e-16)
            degenerateTriangles += 1;
    }
    let invalidNormals = 0;
    for (let offset = 0; offset < geometry.normals.length; offset += 3) {
        const magnitude = Math.hypot(geometry.normals[offset] ?? Number.NaN, geometry.normals[offset + 1] ?? Number.NaN, geometry.normals[offset + 2] ?? Number.NaN);
        if (!Number.isFinite(magnitude) || Math.abs(magnitude - 1) > 1e-4)
            invalidNormals += 1;
    }
    ...
    for (let offset = 0; offset < geometry.weights.length; offset += 4) {
        ...
        if (!Number.isFinite(sum) || Math.abs(sum - 1) > 1e-5 || influences === 0 || influences > 4)
            invalidWeights += 1;
        else
            weightedVertices += 1;
    ...
    const profileModules = blueprint.gates.requiredModules.filter((module) => spec.features?.[module] !== undefined).length;
