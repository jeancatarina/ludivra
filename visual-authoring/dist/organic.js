...
function union(fields, smoothing) {
    return (point) => {
...
function intersection(fields) {
    return (point) => Math.max(...fields.map((field) => field(point)));
}
function ellipsoid(center, radius) {
...
function taperedCapsule(start, end, startRadius, endRadius) {
...
function robeField(centerX, topY, bottomY, topRadius, bottomRadius, topDepth, bottomDepth, seed) {
    return (point) => {
...
function ellipticalTorus(center, radiusX, radiusZ, thickness) {
    return (point) => {
...
function gradient(field, point, epsilon) {
...
function distanceToSegment(point, bone) {
...
function skinWeights(point, skeleton) {
...
function polygonize(field, points, values, color, skeleton, characterHeight, output, epsilon, vertexCache, surface) {
    const intersections = [];
    ...
    const unique = intersections.filter((point, index) => intersections.findIndex((candidate) => length(subtract(point, candidate)) < epsilon * 0.08) === index);
    if (unique.length < 3)
        return;
    ...
    const reference = Math.abs(surfaceNormal[1]) < 0.9 ? [0, 1, 0] : [1, 0, 0];
    ...
    for (let triangle = 1; triangle + 1 < intersections.length; triangle += 1) {
        let vertices = [intersections[0], intersections[triangle], intersections[triangle + 1]];
        const faceNormal = cross(subtract(vertices[1], vertices[0]), subtract(vertices[2], vertices[0]));
        if (length(faceNormal) < epsilon * epsilon * 0.01)
            continue;
        if (dot(faceNormal, surfaceNormal) < 0)
        ...
        for (const vertex of vertices) {
            ...
            if (vertexIndex === undefined) {
                ...
                output.texcoords.push((Math.atan2(vertex[2], vertex[0]) / (Math.PI * 2) + 1) % 1, Math.max(0, Math.min(1, vertex[1] / characterHeight)));
...
function meshField(field, bounds, resolution, color, skeleton, characterHeight, output, vertexCache, surface) {
    ...
    const sample = (x, y, z) => values[(z * countY + y) * countX + x];
    ...
    for (let z = 0; z < countZ - 1; z += 1) {
        for (let y = 0; y < countY - 1; y += 1) {
            for (let x = 0; x < countX - 1; x += 1) {
                ...
                if (cubeValues.every((value) => value > 0) || cubeValues.every((value) => value <= 0))
                    continue;
                for (const tetrahedron of tetrahedra) {
                    polygonize(field, tetrahedron.map((index) => cubePoints[index]), tetrahedron.map((index) => cubeValues[index]), color, skeleton, characterHeight, output, epsilon, vertexCache, surface);
                }
            }
        }
    }
...
export function generateOrganicCharacterSurface(spec, style, skeleton) {
    const output = {
        positions: [],
        normals: [],
        colors: [],
    ...
    if (!fullCoverageOutfit) {
        bodyFields.push(ellipsoid(mix(spine.start, spine.end, 0.55), [
    ...
    if (spec.archetype.head === "human") {
        bodyFields.push(ellipsoid(add(headCenter, [0, -headRadius * 0.16, headRadius * 0.82]), [headRadius * 0.44, headRadius * 0.4, headRadius * 0.46]), ellipsoid(add(headCenter, [-headRadius * 0.48, -headRadius * 0.32, headRadius * 0.48]), [headRadius * 0.5, headRadius * 0.42, headRadius * 0.34]), ellipsoid(add(headCenter, [headRadius * 0.48, -headRadius * 0.32, headRadius * 0.48]), [headRadius * 0.5, headRadius * 0.42, headRadius * 0.34]));
    }
    else {
        bodyFields.push(taperedCapsule(add(headCenter, [0, -headRadius * 0.15, headRadius * 0.62]), add(headCenter, [0, -headRadius * 0.2, headRadius * (1.02 + spec.face.nose * 0.22)]), headRadius * 0.24, headRadius * 0.08));
    }
    if (spec.archetype.head === "goblin" || spec.archetype.head === "orc" || spec.archetype.head === "simple-demon") {
        bodyFields.push(taperedCapsule(add(headCenter, [-headRadius * 0.72, headRadius * 0.12, 0]), add(headCenter, [-headRadius * 1.9, headRadius * 0.22, -headRadius * 0.06]), headRadius * 0.42, headRadius * 0.035), taperedCapsule(add(headCenter, [headRadius * 0.72, headRadius * 0.12, 0]), add(headCenter, [headRadius * 1.9, headRadius * 0.22, -headRadius * 0.06]), headRadius * 0.42, headRadius * 0.035));
    }
    else {
        bodyFields.push(ellipsoid(add(headCenter, [-headRadius * 0.98, 0, -headRadius * 0.02]), [headRadius * 0.3, headRadius * 0.44, headRadius * 0.22]), ellipsoid(add(headCenter, [headRadius * 0.98, 0, -headRadius * 0.02]), [headRadius * 0.3, headRadius * 0.44, headRadius * 0.22]));
    }
    if (!fullCoverageOutfit) {
        for (const name of [
    ...
    }, [extendedPose ? 54 : 38, 58, 32], colorFactor(style.palette[spec.skin] ?? style.palette.skin ?? "#719457"), skeleton, height, output, vertexCache, SURFACE.skin);
    if (spec.archetype.head === "human") {
        ...
        meshField(ellipsoid(noseCenter, [
        ...
        }, [18, 18, 16], colorFactor(style.palette.skinHighlight ?? style.palette[spec.skin] ?? style.palette.skin ?? "#e4a083"), skeleton, height, output, vertexCache, SURFACE.skin);
    }
    if (outfit?.construction === "overalls") {
        ...
        meshField(union(shirtFields, height * 0.018), {
            min: [-height * (extendedPose ? 0.76 : 0.42), height * 0.43, -height * 0.17],
            max: [height * (extendedPose ? 0.76 : 0.42), height * 0.88, height * 0.19]
        }, [extendedPose ? 54 : 42, 34, 28], colorFactor(style.palette[outfit.secondaryRole] ?? "#c84a32"), skeleton, height, output, vertexCache, SURFACE.cloth);
        ...
        overallFields.push(taperedCapsule([-height * 0.09, height * 0.71, height * 0.208], [-height * 0.14, height * 0.82, height * 0.125], height * 0.018, height * 0.015), taperedCapsule([height * 0.09, height * 0.71, height * 0.208], [height * 0.14, height * 0.82, height * 0.125], height * 0.018, height * 0.015));
        meshField(union(overallFields, height * 0.014), {
            min: [-height * 0.23, height * 0.02, -height * 0.15],
            max: [height * 0.23, height * 0.86, height * 0.27]
        }, [36, 54, 28], colorFactor(style.palette[outfit.primaryRole] ?? "#2459b8"), skeleton, height, output, vertexCache, SURFACE.cloth);
        meshField(union([
        ...
        }, [28, 12, 12], colorFactor(style.palette[outfit.trimRole] ?? "#f1b735"), skeleton, height, output, vertexCache, SURFACE.glossy);
    }
    else {
        ...
        const clothingFields = [
            ellipsoid(mix(spine.start, spine.end, 0.5), [
        ...
        if (usesRobe) {
            clothingFields.push(robeField(style.geometry.asymmetry * height * 0.008, hips.end[1] + height * 0.12, height * 0.08, height * 0.145, height * 0.245, height * 0.115, height * 0.18, spec.seed));
        }
        else {
            clothingFields.push(ellipsoid(mix(hips.start, hips.end, 0.48), [
                height * 0.145,
                height * 0.14,
                height * 0.115
            ]));
            for (const name of ["left-upper-arm", "right-upper-arm"]) {
                const bone = boneByName(skeleton, name);
                clothingFields.push(taperedCapsule(bone.start, mix(bone.start, bone.end, usesArmor ? 0.42 : 0.68), bone.radiusStart * (usesArmor ? 1.55 : 1.25), bone.radiusEnd * (usesArmor ? 1.4 : 1.18)));
            }
        }
        ...
        }, [36, 48, 32], colorFactor(style.palette[clothingRole] ?? style.palette.cloth ?? "#176b70"), skeleton, height, output, vertexCache, usesArmor ? SURFACE.hard : SURFACE.cloth);
        meshField(ellipticalTorus([0, hips.start[1] + height * 0.025, height * 0.006], height * 0.155, height * 0.12, height * 0.022), {
            min: [-height * 0.2, hips.start[1] - height * 0.025, -height * 0.15],
            max: [height * 0.2, hips.start[1] + height * 0.075, height * 0.16]
        }, [28, 12, 24], colorFactor(style.palette.leather ?? "#6b472d"), skeleton, height, output, vertexCache, SURFACE.leather);
        meshField(ellipticalTorus([0, spine.end[1] + height * 0.015, spine.end[2]], height * 0.118, height * 0.09, height * 0.014), {
        ...
        }, [26, 11, 22], colorFactor(style.palette.leather ?? "#6b472d"), skeleton, height, output, vertexCache, SURFACE.leather);
        if (usesRobe) {
            meshField(ellipticalTorus([0, height * 0.108, 0], height * 0.238, height * 0.178, height * 0.019), {
                min: [-height * 0.27, height * 0.07, -height * 0.21],
                max: [height * 0.27, height * 0.14, height * 0.21]
            }, [38, 13, 30], colorFactor(style.palette[outfit?.trimRole ?? "accent"] ?? style.palette.bone ?? "#d2c7a5"), skeleton, height, output, vertexCache, SURFACE.hard);
        }
    }
    ...
    if (hair !== undefined && hair.style !== "none") {
        ...
        }, [30, 30, 28], colorFactor(style.palette[hair.colorRole] ?? "#493126"), skeleton, height, output, vertexCache, SURFACE.hair);
    }
    ...
    if (headwear !== undefined) {
        ...
        meshField(union([crownField, brim], headRadius * 0.08), {
        ...
        }, [30, 30, 30], colorFactor(style.palette[headwear.primaryRole] ?? "#2b8f86"), skeleton, height, output, vertexCache, SURFACE.cloth);
        if (headwear.badge !== "none") {
            meshField(ellipsoid(add(headCenter, [0, headRadius * 0.82, headRadius * 0.95]), [headRadius * 0.34, headRadius * 0.38, headRadius * 0.08]), {
            ...
            }, [18, 18, 10], colorFactor(style.palette[headwear.secondaryRole] ?? "#f2d15d"), skeleton, height, output, vertexCache, SURFACE.glossy);
        }
    }
    ...
    if (hands !== undefined) {
        for (const name of ["left-hand", "right-hand"]) {
            ...
            gloveFields.push(ellipsoid(palmCenter, [
                height * blueprint.hands.palmLength,
                height * blueprint.hands.palmWidth,
                height * blueprint.hands.palmDepth
            ]));
            if (hands.style === "gloves") {
                gloveFields.push(ellipsoid(bone.start, [height * 0.032, height * 0.052, height * 0.052]));
            }
            gloveFields.push(taperedCapsule(add(bone.start, scale(direction, -height * 0.012)), mix(bone.start, bone.end, 0.38), bone.radiusStart * 1.4, bone.radiusStart * 1.55));
            for (const spread of [-1, 0, 1]) {
                const base = add(mix(bone.start, bone.end, 0.58), [0, spread * height * 0.018, height * 0.002]);
                const tip = add(add(bone.end, scale(direction, height * (blueprint.hands.fingerLength + (1 - Math.abs(spread)) * 0.004))), [
                    0,
                    spread * height * blueprint.hands.fingerSpread,
                    -Math.abs(spread) * height * 0.002
                ]);
                gloveFields.push(taperedCapsule(base, tip, height * blueprint.hands.fingerRadius, height * blueprint.hands.fingerRadius * 0.78));
            }
            gloveFields.push(taperedCapsule(add(mix(bone.start, bone.end, 0.35), [0, -height * 0.025, height * 0.018]), add(mix(bone.start, bone.end, 0.78), [side * height * 0.012, -height * 0.064, height * 0.025]), height * 0.021, height * 0.014));
            const padding = [height * 0.085, height * 0.105, height * 0.085];
            const handResolution = blueprint.profile === "hero-mascot"
                ? [32, 30, 26]
                : blueprint.profile === "stylized-hero"
                    ? [24, 24, 20]
                    : [16, 16, 14];
            meshField(union(gloveFields, height * 0.0045), {
                min: [
            ...
            }, handResolution, colorFactor(style.palette[hands.colorRole] ?? style.palette[spec.skin] ?? "#f5eee2"), skeleton, height, output, vertexCache, hands.style === "gloves" ? SURFACE.cloth : SURFACE.skin);
        }
    }
    ...
    if (footwear !== undefined && footwear.style !== "bare") {
        ...
        for (const name of ["left-foot", "right-foot"]) {
            ...
            shoeFields.push(taperedCapsule(add(bone.start, [0, height * 0.012, 0]), add(mix(bone.start, bone.end, 0.34), [0, height * 0.012, 0]), height * 0.062, height * 0.057));
        ...
        meshField(union(shoeFields, height * 0.008), {
            min: [-height * 0.22, -height * 0.01, -height * 0.08],
            max: [height * 0.22, height * 0.18, height * 0.23]
        }, [34, 18, 30], colorFactor(style.palette[footwear.colorRole] ?? "#6a3828"), skeleton, height, output, vertexCache, SURFACE.leather);
        meshField(union(soleFields, height * 0.003), {
            min: [-height * 0.22, -height * 0.012, -height * 0.08],
            max: [height * 0.22, height * 0.07, height * 0.24]
        }, [34, 12, 30], colorFactor(style.palette.sole ?? style.palette[footwear.colorRole] ?? "#4f2b22"), skeleton, height, output, vertexCache, SURFACE.leather);
    }
    return output;
}
