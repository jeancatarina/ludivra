...
assert.equal(validated.data.contentFilesChecked, 6);
...
assert.equal(documents["navigation.ember-vault"].profiles[0].maxExpandedNodes, 64,
  "the declared navigation fixture travels in the pack");
assert.equal(documents["world.ember-vault"].regionExtentChunks, 2,
...
assert.equal(documents["mass.ember-vault"].agents.length, 3,
  "the declared Mass population travels in the pack");
assert.ok(
