...
| Próxima entrega | Implementar migração de host verificada por hash sobre o loopback e preparar os adapters WebRTC/Steam sem duplicar a simulação. |
...
| 7 | Persistência, replays e multiplayer player-hosted | `EM ANDAMENTO` | Migração de host verificada por hash e adapters WebRTC/Steam sem duplicar a simulação host-authoritative. |
...
- Sala loopback host-authoritative com protocolo N/N-1, identidade procedural no handshake, inputs de cliente ordenados e limitados, snapshots checksummed, late join, reconexão e recusa de estado enviado por cliente. Capabilities: `network.loopback-room`. Evidência: [kernel/src/network_session.cpp](kernel/src/network_session.cpp), [tests/kernel/kernel_test.cpp](tests/kernel/kernel_test.cpp).

### Falta

- Migração de host verificada por hash e adapters WebRTC/Steam sem duplicar a simulação host-authoritative.
